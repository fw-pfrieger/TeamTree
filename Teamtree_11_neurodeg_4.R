#TeamTree, vers. 11, topic: Neurodegeneration Vers. 4, Oct-13-2021
#Special version, see Pfrieger (2021) Neurodegenerative diseases and cholesterol: seeing the field through the players". Front. Aging Neurosci. https://doi.org/10.3389/fnagi.2021.766587
...
User_dir_read <- "d:\\data\\papers\\TTA_neurodeg\\"
User_dir_save <- "d:\\data\\papers\\TTA_neurodeg\\"
...
File_TT_AC <- paste(User_dir_save, sprintf("%s_TT_AC.csv", Topic),sep="")
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------
#2.2. Workforce analysis using all authors
#----------------------------------------------------------------------------------------------------------------------------------------------------------

#Start timer for this part
start_time_workf <- Sys.time()

#Calculate annual rates of publications
...
D_rates <- data.frame(PY_bins=floor(Pub_rates$mids))
D_rates$PY_bins_norm <- seq_along(D_rates$PY_bins)/length(seq_along(D_rates$PY_bins))
D_rates$pubs_count=Pub_rates$counts
D_rates$pubs_freq <-Pub_rates$counts/sum(Pub_rates$counts)


#Calculate mean annual AC values
D_AC_calc <- DT_pub[,.(AC_mean=mean(Author_count), AC_sum=sum(Author_count)), by=PY]
D_AC_calc <- D_AC_calc[order(PY),]
write.csv(D_AC_calc, file=File_TT_AC, row.names=F)

#Assemble list of all authors
DT_authors <- DT_pub[,Author_list:=lapply(Authors, function(x) x[1:(length(x))])]

if(Pubmed){
Author_list <- toupper(unlist(DT_authors$Author_list))
}else{
#Assemble Col Author family names and initials for WOS data
Author_list_temp <- toupper(unlist(DT_authors$Author_list))
Author_list_famname <- sapply(strsplit(Author_list_temp, ", "), '[', 1)
Author_list_init <-  sapply(strsplit(Author_list_temp, ", "), '[', 2)
Author_list_init <- ifelse(nchar(Author_list_init)<=2,gsub("\\.","",Author_list_init),Author_list_init)
Author_list_init <- ifelse(nchar(Author_list_init)==2& grepl("[a-z]", substring(Author_list_init,2,2)),substring(Author_list_init,1,1),Author_list_init)
Author_list_init <- ifelse(nchar(Author_list_init)>2&!grepl("\\.|\\s|\\-", Author_list_init),substring(Author_list_init,1,1),Author_list_init)
Author_list_init <- ifelse(grepl(".\\.\\s.\\.", Author_list_init),gsub("\\.|\\s", "",Author_list_init),Author_list_init)
Author_list_init <- ifelse(grepl(".\\..\\.", Author_list_init),gsub("\\.", "",Author_list_init),Author_list_init)
Author_list_init <- ifelse(nchar(Author_list_init)>2&grepl(" ", Author_list_init), paste(substring(Author_list_init,1,1),substring(Author_list_init,regexpr(" ",Author_list_init)+1,regexpr(" ",Author_list_init)+1),sep=""),Author_list_init)
Author_list_init <- ifelse(nchar(Author_list_init)>2&grepl("-", Author_list_init), paste(substring(Author_list_init,1,1),substring(Author_list_init,regexpr("-",Author_list_init)+1,regexpr("-",Author_list_init)+1),sep=""),Author_list_init)
Author_list <- paste(Author_list_famname, ifelse(is.na(Author_list_init),"", Author_list_init), sep=" ")
}

#Generate table for all authors repeating year and PubID for each author in article
Author_list_year <- c(rep(DT_authors$PY, DT_authors$Author_count))
Author_list_Pub_ID <- c(rep(DT_authors$Pub_ID, DT_authors$Author_count)) 
Author_list_AC <- c(rep(DT_authors$Author_count, DT_authors$Author_count))

DT_workforce <- data.table("Authors"=Author_list, "PY"=Author_list_year, "Pub_ID"=Author_list_Pub_ID, "AC"=Author_list_AC)

DT_workforce[,Author_index:=1:.N, by=Pub_ID]

#Count publications per all authors per year
DT_wf_all <- DT_workforce[,.N, by=.(PY,Authors)]

#Calculate total number of authors contributing to the field per year
Wf_all_ct <- hist(DT_wf_all$PY, breaks=Rates_bins, plot=F, right=F)
D_rates$wf_all_count <- Wf_all_ct$counts

#Select first authors of publications
DT_workforce_first <- DT_workforce[DT_workforce[,.I[which.min(Author_index)], by=Pub_ID]$V1]

#Count publications per first authors per year
DT_wf_first <- DT_workforce_first[,.N, by=.(PY, Authors)]

#Count first authors contributing to the field per year
Wf_first_ct <- hist(DT_wf_first$PY, breaks=Rates_bins, plot=F, right=F)
D_rates$wf_first_count <- Wf_first_ct$counts


#Select last authors of publications
DT_workforce_last <- DT_workforce[DT_workforce[,.I[which.max(Author_index)], by=Pub_ID]$V1]

#Count publications per last author per year
DT_wf_last <- DT_workforce_last[,.N, by=.(PY, Authors)]

#Count last authors contributing to the field per year (newcomers and established)
Wf_last_ct <- hist(DT_wf_last$PY, breaks=Rates_bins, plot=F, right=F)
D_rates$wf_last_count <- Wf_last_ct$counts


#Select collaborating authors of publications
DT_workforce_col <- DT_workforce[DT_workforce[,.I[which(Author_index > 1 & Author_index < max(Author_index))], by=Pub_ID]$V1]

#Count publications per collaborating author per year
DT_wf_col <- DT_workforce_col[,.N, by=.(PY, Authors)]

#Count collaborating authors contributing to the field per year
Wf_col_ct <- hist(DT_wf_col$PY, breaks=Rates_bins, plot=F, right=F)
D_rates$wf_col_count <- Wf_col_ct$counts


#Count authors with single publications
DT_wf_pub_ct <- DT_workforce[,.N, by=Authors] 
DT_wf_pub_ct_single <- DT_wf_pub_ct[N==1]

#Determine annual rate of authors with single publications
DT_wf_all_single <- DT_wf_all[Authors %in% DT_wf_pub_ct_single$Authors]
Wf_all_single <- hist(DT_wf_all_single$PY, breaks=Rates_bins, plot=F, right=F)
D_rates$wf_all_single_count <- Wf_all_single$counts


#Analyse newcomers
#Determine publication rates of newcomers and save to database
DT_wf_enter <- DT_workforce[DT_workforce[,.I[PY==min(PY)], keyby=Authors]$V1]
Wf_enter_pub <- DT_wf_enter[,.N, by=.(PY,Pub_ID)]
Wf_enter_pub_count <- hist(Wf_enter_pub$PY, breaks=Rates_bins, plot=F, right=F)
D_rates$wf_enter_pub_count <- Wf_enter_pub_count$counts

#Determine number of authors entering the field per year
Wf_enter_author <- DT_wf_enter[,.N, keyby=.(Authors,PY)]
Wf_enter_author_count <- hist(Wf_enter_author$PY, breaks=Rates_bins, plot=F, right=F)
D_rates$wf_enter_author_count_count <- Wf_enter_author_count$counts

#Analyse leaving authors
#Determine publication rates of authors leaving the field
DT_wf_exit <- DT_workforce[DT_workforce[,.I[PY==max(PY)], keyby=Authors]$V1]
Wf_exit_pub <- DT_wf_exit[,.N, by=.(PY,Pub_ID)]
Wf_exit_pub_count <- hist(Wf_exit_pub$PY, breaks=Rates_bins, plot=F, right=F)
D_rates$wf_exit_pub_count <- Wf_exit_pub_count$counts

#Determine number of authors leaving the field per year
Wf_exit_author <- DT_wf_exit[,.N, keyby=.(Authors,PY)]
Wf_exit_author_count <- hist(Wf_exit_author$PY, breaks=Rates_bins, plot=F, right=F)
D_rates$wf_exit_author_count <- Wf_exit_author_count$counts

#Calculate time required for this part
end_time_workf <- Sys.time()
Duration_workf <- end_time_workf - start_time_workf


#----------------------------------------------------------------------------------------------------------------------------------------------------------
#2.3. Analysis of the publication record per author
#----------------------------------------------------------------------------------------------------------------------------------------------------------

#Remove ambiguous names
if(RemAmbig){DT_pub <- DT_pub[!grepl(Ambig_names, Last)]} else{}
...
#Calculate annual rates of last authors entering the field
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------
#2.4. Analysis of genealogic connections between authors based on first-last author pairs considered as ancestor-offspring.
#----------------------------------------------------------------------------------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------
#2.5. Analysis of collaborative connections between authors based on co-authorship (2nd to nth-1 of article authors)
#----------------------------------------------------------------------------------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------
#2.6. Calculation of TeamTree product (TTP), a citation-independent measure of author performance, based on the product of PC, OC and CC values.
#----------------------------------------------------------------------------------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------
#2.7. Analysis of field-specific publication and author entry rates in different categories
#----------------------------------------------------------------------------------------------------------------------------------------------------------
...
Topic_col <- c("black", "#FFA500", "#87CEEB", "#009e73", "#ff0080", "#996633")
col_norm <- Topic_col[4]
...
Plot_height_PY <- (PY_max-PY_min)*2.5
Plot_width_AI <- AI_max/17

Plot_width_AI_norm <- 170
Plot_height_PY_norm <- 130
...
if(Workf_Analysis){
Plot_workf_width <- 200
Plot_workf_height <- Plot_workf_width/1.6180

File_P_workf_pub <- sprintf("%s_P_workf_pub.tiff", Topic)
File_P_workf_auth <- sprintf("%s_P_workf_auth.tiff", Topic)

#Read data from subfields
Topic_1 <- "pub_neurodeg_2_alz"
Topic_2 <- "pub_neurodeg_2_multscl"
Topic_3 <- "pub_neurodeg_2_park"
Topic_4 <- "pub_neurodeg_2_amyo"
Topic_5 <- "pub_neurodeg_2_hunt"

Topic_col <- c("black", "#FFA500", "#87CEEB", "#009e73", "#ff0080", "#996633")

#Read TeamTree data
File_TT_data_1 <- paste(User_dir_save, sprintf("%s_TT_data.csv", Topic_1),sep="\\")
D_all_1 <- read.csv(file=File_TT_data_1)
D_all_1$Topic <- 1

File_TT_data_2 <- paste(User_dir_save, sprintf("%s_TT_data.csv", Topic_2),sep="\\")
D_all_2 <- read.csv(file=File_TT_data_2)
D_all_2$Topic <- 2

File_TT_data_3 <- paste(User_dir_save, sprintf("%s_TT_data.csv", Topic_3),sep="\\")
D_all_3 <- read.csv(file=File_TT_data_3)
D_all_3$Topic <- 3

File_TT_data_4 <- paste(User_dir_save, sprintf("%s_TT_data.csv", Topic_4),sep="\\")
D_all_4 <- read.csv(file=File_TT_data_4)
D_all_4$Topic <- 4

File_TT_data_5 <- paste(User_dir_save, sprintf("%s_TT_data.csv", Topic_5),sep="\\")
D_all_5 <- read.csv(file=File_TT_data_5)
D_all_5$Topic <- 5
...
D_data_all <- rbind(D_all_0, D_all_1,D_all_2,D_all_3,D_all_4,D_all_5)


#Read TeamTree rates
File_TT_rates_1 <- paste(User_dir_save, sprintf("%s_TT_rates.csv", Topic_1),sep="\\")
D_rates_1 <- read.csv(file=File_TT_rates_1)
D_rates_1$Topic <- 1

File_TT_rates_2 <- paste(User_dir_save, sprintf("%s_TT_rates.csv", Topic_2),sep="\\")
D_rates_2 <- read.csv(file=File_TT_rates_2)
D_rates_2$Topic <- 2

File_TT_rates_3 <- paste(User_dir_save, sprintf("%s_TT_rates.csv", Topic_3),sep="\\")
D_rates_3 <- read.csv(file=File_TT_rates_3)
D_rates_3$Topic <- 3

File_TT_rates_4 <- paste(User_dir_save, sprintf("%s_TT_rates.csv", Topic_4),sep="\\")
D_rates_4 <- read.csv(file=File_TT_rates_4)
D_rates_4$Topic <- 4

File_TT_rates_5 <- paste(User_dir_save, sprintf("%s_TT_rates.csv", Topic_5),sep="\\")
D_rates_5 <- read.csv(file=File_TT_rates_5)
D_rates_5$Topic <- 5

D_rates$Topic <- 0

D_rates_all <- rbind(D_rates, D_rates_1, D_rates_2, D_rates_3, D_rates_4, D_rates_5) 

#Number of papers per year
P_workf <- ggplot()
P_workf <- P_workf + geom_step(data=D_rates[-nrow(D_rates),], aes(x=PY_bins, y = pubs_count), size=1.5, color="black")
#P_workf <- P_workf + geom_step(data=D_rates_1[-nrow(D_rates_1),], aes(x=PY_bins, y = pubs_count), size=1.25, color="#FFA500")
#P_workf <- P_workf + geom_step(data=D_rates_2[-nrow(D_rates_2),], aes(x=PY_bins, y = pubs_count), size=1.25, color="#87CEEB")
#P_workf <- P_workf + geom_step(data=D_rates_3[-nrow(D_rates_3),], aes(x=PY_bins, y = pubs_count), size=1.25, color="#009e73")
#P_workf <- P_workf + geom_step(data=D_rates_4[-nrow(D_rates_4),], aes(x=PY_bins, y = pubs_count), size=1.25, color="#ff0080")
#P_workf <- P_workf + geom_step(data=D_rates_5[-nrow(D_rates_5),], aes(x=PY_bins, y = pubs_count), size=1.25, color="#996633")
P_workf <- P_workf + theme_bw()
P_workf <- P_workf + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black", size = Line_width))
P_workf <- P_workf + theme(axis.ticks.length = unit(Tick_length, "cm"), axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_line(color = "black", size = Line_width))
P_workf <- P_workf + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
P_workf <- P_workf + scale_x_continuous(limits= c(PY_min,PY_max), breaks = seq(1700, 2040, by = Tick_years))
#P_workf <- P_workf + scale_y_continuous(limits= c(1950,2021), breaks = seq(0, 10000, by = 500))
P_workf <- P_workf + scale_y_continuous(breaks = seq(0, 10000, by = 50))
if(show_plots){P_workf}
if(save_plots){ggsave(File_P_workf_pub, path = User_dir_save, width = Plot_workf_width, height = Plot_workf_height, units = "mm", dpi=300)}

#Number of authors per year (all authors, last authors and authors with single contributions)
P_workf <- ggplot()
P_workf <- P_workf + geom_step(data=D_rates[-nrow(D_rates),], aes(x=PY_bins, y = wf_all_count), size=1.5, color="black")
#P_workf <- P_workf + geom_step(data=D_rates[-nrow(D_rates),], aes(x=PY_bins, y = pubs_count), size=1.25, color="orange")
P_workf <- P_workf + theme_bw()
P_workf <- P_workf + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black", size = Line_width))
P_workf <- P_workf + theme(axis.ticks.length = unit(Tick_length, "cm"), axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_line(color = "black", size = Line_width))
P_workf <- P_workf + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
P_workf <- P_workf + scale_x_continuous(limits= c(PY_min,PY_max), breaks = seq(1700, 2040, by = Tick_years))
P_workf <- P_workf + scale_y_continuous(breaks = seq(0, 10000, by = 500))
if(show_plots){P_workf}
if(save_plots){ggsave(File_P_workf_auth, path = User_dir_save, width = Plot_workf_width, height = Plot_workf_height, units = "mm", dpi=300)}


#Plot annual mean count of authors per article for different fields
D_AC_calc$Topic <- 0

#Filenames to read TeamTree data
File_TT_AC_1 <- paste(User_dir_save, sprintf("%s_TT_AC.csv", Topic_1),sep="\\")
D_AC_1 <- read.csv(file=File_TT_AC_1)
D_AC_1$Topic <- 1

File_TT_AC_2 <- paste(User_dir_save, sprintf("%s_TT_AC.csv", Topic_2),sep="\\")
D_AC_2 <- read.csv(file=File_TT_AC_2)
D_AC_2$Topic <- 2

File_TT_AC_3 <- paste(User_dir_save, sprintf("%s_TT_AC.csv", Topic_3),sep="\\")
D_AC_3 <- read.csv(file=File_TT_AC_3)
D_AC_3$Topic <- 3

File_TT_AC_4 <- paste(User_dir_save, sprintf("%s_TT_AC.csv", Topic_4),sep="\\")
D_AC_4 <- read.csv(file=File_TT_AC_4)
D_AC_4$Topic <- 4

File_TT_AC_5 <- paste(User_dir_save, sprintf("%s_TT_AC.csv", Topic_5),sep="\\")
D_AC_5 <- read.csv(file=File_TT_AC_5)
D_AC_5$Topic <- 5

D_AC_all <- rbind(D_AC_calc, D_AC_1, D_AC_2, D_AC_3, D_AC_4, D_AC_5) 


#Plot annual mean author count per year
File_P_AC_mean <- sprintf("%s_P_AC_mean.tiff", Topic)

P_AC_mean <- ggplot()
#P_AC_mean <- P_AC_mean + geom_step(data=D_AC_all, aes(x=PY, y=AC_mean, group=as.factor(Topic),color=as.factor(Topic)), size=1.5)
P_AC_mean <- P_AC_mean + geom_line(data=D_AC_all[which(D_AC_all$Topic==0),], aes(x=PY, y=AC_mean, group=as.factor(Topic),color=as.factor(Topic)), size=1.5)
P_AC_mean <- P_AC_mean + scale_colour_manual(values=Topic_col)
P_AC_mean <- P_AC_mean + theme_bw()
P_AC_mean <- P_AC_mean + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black", size = Line_width))
P_AC_mean <- P_AC_mean + theme(axis.ticks.length = unit(Tick_length, "cm"), axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_line(color = "black", size = Line_width))
P_AC_mean <- P_AC_mean + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
P_AC_mean <- P_AC_mean + theme(legend.position="none")
P_AC_mean <- P_AC_mean + scale_x_continuous(limits= c(PY_min,PY_max), breaks = seq(1700, 2040, by = Tick_years))
P_AC_mean <- P_AC_mean + scale_y_continuous(limits=c(0,9), breaks = seq(0, 90, by = 3))
if(show_plots){P_AC_mean}
if(save_plots){ggsave(File_P_AC_mean, path = User_dir_save, width = Plot_workf_width, height = Plot_workf_height, units = "mm", dpi=300)}

#Annual counts of authors entering and leaving the field
D_rates$wf_author_sum <- D_rates$wf_enter_author_count-D_rates$wf_exit_author_count
D_rates$wf_author_cum_sum <- cumsum(D_rates$wf_author_sum)

File_P_growth <- sprintf("%s_P_growth.tiff", Topic)

D_rates_growth <- D_rates[1:(nrow(D_rates)-3),]

P_growth <- ggplot()
P_growth <- P_growth + geom_col(data=D_rates_growth, aes(x=PY_bins, y = wf_enter_author_count_count), color=NA, fill="green3")
P_growth <- P_growth + geom_col(data=D_rates_growth, aes(x=PY_bins, y = wf_exit_author_count*-1), color=NA, fill="red3")
P_growth <- P_growth + geom_col(data=D_rates_growth, aes(x=PY_bins, y = wf_all_single_count*-1), color=NA, fill="gray80")
P_growth <- P_growth + geom_col(data=D_rates_growth, aes(x=PY_bins, y = wf_all_single_count), color=NA, fill="gray80")
P_growth <- P_growth + geom_step(data=D_rates_growth[which(D_rates_growth$wf_author_sum >0),], aes(x=PY_bins, y = wf_author_sum), size=1.5, color="black")
P_growth <- P_growth + geom_step(data=D_rates_growth[which(D_rates_growth$wf_author_sum <0),], aes(x=PY_bins, y = wf_author_sum), size=1.2, color="orange")
P_growth <- P_growth + geom_hline(yintercept=0, color="black", size=0.5)
#P_growth <- P_growth + geom_step(data=D_rates_growth, aes(x=PY_bins, y = wf_author_sum), size=1.5, color="black")
...
P_growth <- P_growth + scale_y_continuous(limits=c(-1600, 1600), breaks=seq(-50000,50000, by = 500))
if(show_plots){P_growth}
if(save_plots){ggsave(File_P_growth, path = User_dir_save, height = Plot_workf_height, width = Plot_workf_width, units = "mm", dpi=300)}

D_single_fract <- D_rates_all[which(D_rates_all$wf_all_count >0),c(1,5,9,69)]

D_single_fract$wf_all_single_fract <- D_single_fract$wf_all_single_count/D_single_fract$wf_all_count

aggregate(D_single_fract$wf_all_single_fract, by=list(D_single_fract$Topic), FUN=mean)

File_P_single_fract <- sprintf("%s_P_single_fract.tiff", Topic)

P_single_fract <- ggplot()
#P_single_fract <- P_single_fract + geom_point(data=D_single_fract, aes(x=Topic+1, y=wf_all_single_fract, group=as.factor(Topic), color=as.factor(Topic)))
P_single_fract <- P_single_fract + stat_summary(data=D_single_fract, aes(x=Topic+1, y=wf_all_single_fract, group=as.factor(Topic), color=as.factor(Topic)), fun = mean, geom = "bar", fill=NA, size=1.5, width=0.6)
#P_single_fract <- P_single_fract + stat_summary(data=D_single_fract, aes(x=Topic+1, y=wf_all_single_fract, group=as.factor(Topic), color=as.factor(Topic)), fun.data = mean_se, geom = "errorbar", size=1.5, width=0.6)
P_single_fract <- P_single_fract + stat_summary(data=D_single_fract, aes(x=Topic+1, y=wf_all_single_fract, group=as.factor(Topic), color=as.factor(Topic)), fun = mean, fun.min = function(x) mean(x) - sd(x), fun.max = function(x) mean(x) + sd(x), geom = "pointrange", size=1.5)
#P_single_fract <- P_single_fract + geom_bar(data=D_single_fract, aes(x=Topic+1, y=wf_all_single_fract, group=as.factor(Topic), color=as.factor(Topic)), position="dodge", stat = "summary", fun = "median", size=1.5, fill=NA, width=0.5)
P_single_fract <- P_single_fract + scale_colour_manual(values=Topic_col)
P_single_fract <- P_single_fract + theme_bw()
P_single_fract <- P_single_fract + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black", size = Line_width))
P_single_fract <- P_single_fract + theme(axis.ticks.length = unit(Tick_length, "cm"), axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_line(color = "black", size = Line_width))
P_single_fract <- P_single_fract + theme(legend.position="none")
P_single_fract <- P_single_fract + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
P_single_fract <- P_single_fract + scale_x_continuous(limits = c(0.5, 6.5), breaks = seq(1, 6, by = 1))
P_single_fract <- P_single_fract + scale_y_continuous(limits=c(0, 1), breaks=seq(0,1, by = 0.25))
if(show_plots){P_single_fract}
if(save_plots){ggsave(File_P_single_fract, path = User_dir_save, height = Plot_workf_height/2, width = Plot_workf_width/1.5, units = "mm", dpi=300)}

} else{}

#Calculate global workforce counts
D_workf_count <- data.frame(Pub_count=c(sum(D_rates$pubs_count),sum(D_rates_1$pubs_count),sum(D_rates_2$pubs_count),sum(D_rates_3$pubs_count),sum(D_rates_4$pubs_count),sum(D_rates_5$pubs_count)))
D_workf_count$Topic <- c(0,1,2,3,4,5)
D_workf_count$Auth_all_count <- c(sum(D_rates$wf_all_count),sum(D_rates_1$wf_all_count),sum(D_rates_2$wf_all_count),sum(D_rates_3$wf_all_count),sum(D_rates_4$wf_all_count),sum(D_rates_5$wf_all_count))

D_sgl_all_count <- aggregate(D_rates_all$wf_all_single_count, by=list(Topic=D_rates_all$Topic), FUN=sum)

D_workf_count$Sgl_all_count <- D_sgl_all_count$x
D_workf_count$Ratio_sgl_all <- D_workf_count$Sgl_all_count/D_workf_count$Auth_all_count

D_author_count <- D_data_all[which(D_data_all$PC==1),]
D_author_count <- aggregate(D_data_all$Author_name, by=list(Topic=D_data_all$Topic), FUN=length)
D_workf_count$Auth_count <- D_author_count$x

D_fam_count <- D_data_all[which(D_data_all$AG==1 & D_data_all$Family_size > 0),]
D_fam_size <- aggregate(D_fam_count$Family_size, by=list(Topic=D_fam_count$Topic), FUN=sum)
D_workf_count$Fam_size <- D_fam_size$x

D_col_count <- D_data_all[which(D_data_all$degree_all > 0),]
D_col_size <- aggregate(D_col_count$Author_name, by=list(Topic=D_col_count$Topic), FUN=length)
D_workf_count$Col_size <- D_col_size$x
D_workf_count$Color <- Topic_col

D_TTP_count <- D_data_all[which(D_data_all$PCxOCxCC > 0),]
D_TTP_size <- aggregate(D_TTP_count$Author_name, by=list(Topic=D_TTP_count$Topic), FUN=length)
D_TTT_size_3 <- data.frame("Topic"=3, "x"=0)
D_TTP_size <- rbind(D_TTP_size,D_TTT_size_3)

D_TTP_size <- D_TTP_size[order(D_TTP_size$Topic),]

D_workf_count$TTP_size <- D_TTP_size$x

File_P_workf_count <- sprintf("%s_P_workf_count.tiff", Topic)

P_workf_count <- ggplot(data=D_workf_count)
P_workf_count <- P_workf_count + geom_point(aes(x=Pub_count, y=Auth_all_count, shape=as.factor(Topic), color=Color), size=12, fill = NA, stroke = 2.5)
#P_workf_count <- P_workf_count + geom_point(aes(x=Pub_count, y=Sgl_all_count, shape=as.factor(Topic), color=Color), size=12, fill = NA, stroke = 2.5)
#P_workf_count <- P_workf_count + geom_point(aes(x=Pub_count, y=Auth_count, shape=as.factor(Topic), color=Color), size=12, fill = NA, stroke = 2.5)
#P_workf_count <- P_workf_count + geom_point(aes(x=Pub_count, y=Col_size, shape=as.factor(Topic), color=Color), size=12, fill = NA, stroke = 2.5)
#P_workf_count <- P_workf_count + geom_point(aes(x=Pub_count, y=Fam_size, shape=as.factor(Topic), color=Color), size=12, fill = NA, stroke = 2.5)
#P_workf_count <- P_workf_count + geom_point(aes(x=Pub_count, y=TTP_size, shape=as.factor(Topic), color=Color), size=12, fill = NA, stroke = 2.5)
P_workf_count <- P_workf_count + scale_colour_identity()
P_workf_count <- P_workf_count + scale_shape_manual(values=c(21,25,24,21,23,22))
P_workf_count <- P_workf_count + theme_bw()
P_workf_count <- P_workf_count + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black", size = Line_width))
P_workf_count <- P_workf_count + theme(axis.ticks.length = unit(Tick_length, "cm"), axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_line(color = "black", size = Line_width))
P_workf_count <- P_workf_count + theme(legend.position="none")
P_workf_count <- P_workf_count + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
P_workf_count <- P_workf_count + scale_x_continuous(limits = c(0, 5000), breaks = seq(0, 5000, by = 1000))
P_workf_count <- P_workf_count + scale_y_continuous(limits = c(0, 30000), breaks=seq(0,30000, by = 5000))
#P_workf_count <- P_workf_count + scale_y_continuous(limits = c(0, 9000), breaks=seq(0,30000, by = 3000))
#P_workf_count <- P_workf_count + scale_y_continuous(limits = c(0, 1700), breaks=seq(0,30000, by = 400))
#P_workf_count <- P_workf_count + scale_y_continuous(limits = c(0, 700), breaks=seq(0,30000, by = 200))
#P_workf_count <- P_workf_count + scale_y_continuous(limits = c(0, 210), breaks=seq(0,30000, by = 100))
if(show_plots){P_workf_count}
#if(save_plots){ggsave(File_P_workf_count, path = User_dir_save, width = Plot_workf_height, height = Plot_workf_height*0.9, units = "mm", dpi=300)}
if(save_plots){ggsave(File_P_workf_count, path = User_dir_save, width = 150, height = 150, units = "mm", dpi=300)}

#Calculate ratios author/pub etc.
D_workf_count$Ratio_auth <- D_workf_count$Auth_count/D_workf_count$Auth_all_count
D_workf_count$Ratio_fam_pub <- D_workf_count$Fam_size/D_workf_count$Auth_count
D_workf_count$Ratio_col_pub <- D_workf_count$Col_size/D_workf_count$Auth_count

File_P_wf_fract <- sprintf("%s_P_wf_fract.tiff", Topic)

P_wf_fract <- ggplot(data=D_workf_count[-1,])
P_wf_fract <- P_wf_fract + geom_point(aes(x=1, y=Ratio_sgl_all, group=as.factor(Topic), color=as.factor(Topic), shape=as.factor(Topic)), size=12, stroke=2.5)
#P_wf_fract <- P_wf_fract + geom_point(aes(x=2, y=Ratio_auth, group=as.factor(Topic), color=as.factor(Topic), shape=as.factor(Topic)), size=12, stroke=2.5)
P_wf_fract <- P_wf_fract + geom_point(aes(x=2, y=Ratio_col_pub, group=as.factor(Topic), color=as.factor(Topic), shape=as.factor(Topic)), size=12, stroke=2.5)
P_wf_fract <- P_wf_fract + geom_point(aes(x=3, y=Ratio_fam_pub, group=as.factor(Topic), color=as.factor(Topic), shape=as.factor(Topic)), size=12, stroke=2.5)
P_wf_fract <- P_wf_fract + stat_summary(aes(x=1, y=Ratio_sgl_all), fun = mean, fun.min = function(x) mean(x) - sd(x), fun.max = function(x) mean(x) + sd(x), geom = "pointrange", size=1.5)
#P_wf_fract <- P_wf_fract + stat_summary(aes(x=2, y=Ratio_auth), fun = mean, fun.min = function(x) mean(x) - sd(x), fun.max = function(x) mean(x) + sd(x), geom = "pointrange", size=1.5)
P_wf_fract <- P_wf_fract + stat_summary(aes(x=2, y=Ratio_col_pub), fun = mean, fun.min = function(x) mean(x) - sd(x), fun.max = function(x) mean(x) + sd(x), geom = "pointrange", size=1.5)
P_wf_fract <- P_wf_fract + stat_summary(aes(x=3, y=Ratio_fam_pub), fun = mean, fun.min = function(x) mean(x) - sd(x), fun.max = function(x) mean(x) + sd(x), geom = "pointrange", size=1.5)
P_wf_fract <- P_wf_fract + scale_colour_manual(values=Topic_col[-1])
P_wf_fract <- P_wf_fract + scale_shape_manual(values=c(25,24,21,23,22))
P_wf_fract <- P_wf_fract + theme_bw()
P_wf_fract <- P_wf_fract + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black", size = Line_width))
P_wf_fract <- P_wf_fract + theme(axis.ticks.length = unit(Tick_length, "cm"), axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_line(color = "black", size = Line_width))
P_wf_fract <- P_wf_fract + theme(legend.position="none")
P_wf_fract <- P_wf_fract + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
P_wf_fract <- P_wf_fract + scale_x_continuous(limits = c(0.5, 3.5), breaks = seq(1, 6, by = 1))
P_wf_fract <- P_wf_fract + scale_y_continuous(limits=c(0, 1), breaks=seq(0,1, by = 0.2))
if(show_plots){P_wf_fract}
if(save_plots){ggsave(File_P_wf_fract, path = User_dir_save, height = Plot_workf_height, width = Plot_workf_height, units = "mm", dpi=300)}
...
D_all$Author_index_norm <- D_all$Author_index/max(abs(D_all$Author_index))
...
D_PC_bottom <- D_PC_sorted[Top_n+1:nrow(D_PC_sorted)-10,]
...
File_P_sub_PC_names <- sprintf("%s_P_sub_PC_names.tiff", Topic)
...
P_pub <- P_pub + scale_size_continuous(range = c(P_pub_size_min, P_pub_size_max), breaks = c(P_pub_size_min, P_pub_size_max))
P_pub <- P_pub + geom_text_repel(data = D_PC_top_minus, aes(x = Author_index, y = PY_start, label = PC_rank, color = Author_color), nudge_x = Tick_AI*-1, nudge_y = -5, hjust = 0, vjust = 0, size = Label_size, inherit.aes=FALSE)
P_pub <- P_pub + geom_text_repel(data = D_PC_top_plus, aes(x = Author_index, y = PY_start, label = PC_rank, color = Author_color), nudge_x = Tick_AI*1, nudge_y = -5, hjust = 0, vjust = 0, size = Label_size, inherit.aes=FALSE)
...
File_P_pub_norm <- sprintf("%s_P_pub_norm.tiff", Topic)

#Visualize publication records of authors with normalized author indices
P_pub_norm <- ggplot()
P_pub_norm <- P_pub_norm + geom_line(data = D_all, aes(x = Author_index_norm, y = PY_start), size=0.5, color = "black")
P_pub_norm <- P_pub_norm + geom_point(data = D_PC_bottom, aes(x = Author_index_norm, y = PY_start, size = PC), color=  col_norm, fill = "gray80", shape = 21, alpha=Alpha_all)
P_pub_norm <- P_pub_norm + geom_point(data = D_PC_top, aes(x = Author_index_norm, y = PY_start, size = PC), fill = col_norm, shape = 21, color="black", alpha=Alpha_all, stroke = 1)
P_pub_norm <- P_pub_norm + scale_size_continuous(range = c(min(D_PC_bottom$PC), max(D_PC_top$PC)), breaks = c(min(D_PC_bottom$PC), max(D_PC_top$PC)))
#P_pub_norm <- P_pub_norm + geom_text_repel(data = D_PC_top_minus, aes(x = Author_index_norm, y = PY_start, label = PC_rank), color="black", nudge_x = -0.3, nudge_y = -7, hjust = 0, vjust = 0, size = Label_size, inherit.aes=FALSE)
#P_pub_norm <- P_pub_norm + geom_text_repel(data = D_PC_top_plus, aes(x = Author_index_norm, y = PY_start, label = PC_rank), color="black", nudge_x = 0.4, nudge_y = -7, hjust = 0, vjust = 0, size = Label_size, inherit.aes=FALSE)

P_pub_norm <- P_pub_norm + geom_text_repel(data = D_PC_top_minus, aes(x = Author_index_norm, y = PY_start, label = PC_rank), color="black", nudge_x = -0.3, nudge_y = -7, hjust = 0, vjust = 1, size = Label_size, inherit.aes=FALSE)
P_pub_norm <- P_pub_norm + geom_text_repel(data = D_PC_top_plus, aes(x = Author_index_norm, y = PY_start, label = PC_rank), color="black", nudge_x = 0.25, nudge_y = -17, hjust = 0, vjust = 0, size = Label_size, inherit.aes=FALSE)

#P_pub_norm <- P_pub_norm + geom_text_repel(data = D_PC_top_minus_3, aes(x = Author_index_norm, y = PY_start, label = PC_rank), color="black", nudge_x = -0.3, nudge_y = -5, hjust = 0, vjust = 1, size = Label_size, inherit.aes=FALSE)
#P_pub_norm <- P_pub_norm + geom_text_repel(data = D_PC_top_plus_3, aes(x = Author_index_norm, y = PY_start, label = PC_rank), color="black", nudge_x = 0.1, nudge_y = -5, hjust = 0, vjust = 1, size = Label_size, inherit.aes=FALSE)

#P_pub_norm <- P_pub_norm + geom_text_repel(data = D_PC_top_minus_4, aes(x = Author_index_norm, y = PY_start, label = PC_rank), color="black", nudge_x = -0.3, nudge_y = -5, hjust = 0, vjust = 1, size = Label_size, inherit.aes=FALSE)
#P_pub_norm <- P_pub_norm + geom_text_repel(data = D_PC_top_plus_4, aes(x = Author_index_norm, y = PY_start, label = PC_rank), color="black", nudge_x = 0.1, nudge_y = -5, hjust = 0, vjust = 1, size = Label_size, inherit.aes=FALSE)

#P_pub_norm <- P_pub_norm + geom_text_repel(data = D_PC_top_minus_5, aes(x = Author_index_norm, y = PY_start, label = PC_rank), color="black", nudge_x = -0.3, nudge_y = -5, hjust = 0, vjust = 1, size = Label_size, inherit.aes=FALSE)
#P_pub_norm <- P_pub_norm + geom_text_repel(data = D_PC_top_plus_5, aes(x = Author_index_norm, y = PY_start, label = PC_rank), color="black", nudge_x = 0.2, nudge_y = -17, hjust = 0, vjust = 0, size = Label_size, inherit.aes=FALSE)

P_pub_norm <- P_pub_norm + theme_bw()
P_pub_norm <- P_pub_norm + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black", size = Line_width))
P_pub_norm <- P_pub_norm + theme(legend.text = element_text(size = Legend_text_size), legend.position = c(0.85,0.3), legend.title = element_blank(), legend.background=element_rect(fill=NA), legend.key=element_blank()) 
P_pub_norm <- P_pub_norm + theme(axis.ticks.length = unit(Tick_length, "cm"), axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_line(color = "black", size = Line_width))
P_pub_norm <- P_pub_norm + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
P_pub_norm <- P_pub_norm + scale_x_continuous(limits = c(-1, 1), breaks = seq(-1,1, by=0.2))
P_pub_norm <- P_pub_norm + scale_y_continuous(limits = c(PY_min,PY_max), breaks = seq(1700, 2040, by = Tick_years))
if(show_plots){P_pub_norm}
if(save_plots){ggsave(File_P_pub_norm, path = User_dir_save, height = Plot_height_PY_norm, width = Plot_width_AI_norm, units = "mm", dpi=300)}
...
if(show_plots){P_PC}
if(save_plots){ggsave(File_P_PC, path = User_dir_save, height = Plot_div_height*1.25, width = Plot_width_AI, units = "mm", dpi=300)}
...
if(save_plots){ggsave(File_P_PC_histo, path = User_dir_save, height = Plot_div_height*1.25, width = Plot_histo_width, units = "mm", dpi=300)}
...
D_OC_bottom <- D_OC_sorted[-(1:Top_n),]
...
P_off <- P_off + geom_text_repel(data = D_OC_top_minus, aes(x = Author_index, y = PY_start, label = OC_rank, color = Author_color), nudge_x = Tick_AI*-1, nudge_y = -10, hjust = 0, vjust = 0, size = Label_size, inherit.aes=FALSE)
...
P_off <- P_off + scale_size_continuous(range = c(P_OC_top_size_norm_min/4, P_OC_top_size_norm_max/4), breaks=c(P_OC_top_min, P_OC_top_max))
...
P_off <- P_off + theme(legend.text = element_text(size = Legend_text_size), legend.background = element_blank(), legend.position =  c(0.8, 0.3), legend.title = element_blank(), legend.key=element_blank())
...
File_P_off_norm <- sprintf("%s_P_off_norm.tiff", Topic)

#Offspring connections with normalized AI
P_off_norm <- ggplot()
P_off_norm <- P_off_norm + geom_line(data = D_all, aes(x = Author_index_norm, y = PY_start), size=0.5, color = "black")
#P_off_norm <- P_off_norm + geom_point(data = D_OC_bottom, aes(x = Author_index_norm, y = PY_start, size = OC), color = col_norm, fill = "gray80", shape = 21, stroke = 1, alpha=Alpha_all)
P_off_norm <- P_off_norm + geom_point(data = D_OC_top, aes(x = Author_index_norm, y = PY_start, size = OC), fill = col_norm, shape = 21, color="black", alpha=Alpha_all, stroke = 1)
#P_off_norm <- P_off_norm + scale_size_continuous(range = c(min(D_OC_bottom$OC)*5, max(D_OC_top$OC)*5), breaks = c(min(D_OC_bottom$OC), max(D_OC_top$OC)))
P_off_norm <- P_off_norm + scale_size_continuous(range = c(5, 5), breaks = c(1, max(D_OC_top$OC)))
P_off_norm <- P_off_norm + geom_text_repel(data = D_OC_top_minus, aes(x = Author_index_norm, y = PY_start, label = OC_rank), color = "black", nudge_x = -0.4, nudge_y = -10, hjust = 0, vjust = 0, size = Label_size, inherit.aes=FALSE)
P_off_norm <- P_off_norm + geom_text_repel(data = D_OC_top_plus, aes(x = Author_index_norm, y = PY_start, label = OC_rank), color = "black", nudge_x = 0.2, nudge_y = -4, hjust = 0, vjust = 0.5, size = Label_size, inherit.aes=FALSE)
P_off_norm <- P_off_norm + scale_fill_identity()
P_off_norm <- P_off_norm + scale_colour_identity()
P_off_norm <- P_off_norm + theme_bw()
P_off_norm <- P_off_norm + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black", size = Line_width))
P_off_norm <- P_off_norm + theme(legend.text = element_text(size = Legend_text_size), legend.background = element_blank(), legend.position =  c(0.85, 0.3), legend.title = element_blank(), legend.key=element_blank())
P_off_norm <- P_off_norm + theme(axis.ticks.length = unit(Tick_length, "cm"), axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_line(color = "black", size = Line_width))
P_off_norm <- P_off_norm + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
P_off_norm <- P_off_norm + scale_x_continuous(limits = c(-1, 1), breaks = seq(-1,1, by=0.2))
P_off_norm <- P_off_norm + scale_y_continuous(limits = c(PY_min,PY_max), breaks = seq(1700, 2040, by = Tick_years))
if(show_plots){P_off_norm}
if(save_plots){ggsave(File_P_off_norm, path = User_dir_save, height = Plot_height_PY_norm, width = Plot_width_AI_norm, units = "mm", dpi=300)}
...
#D_OC_top <- D_OC_top[1:5,]
#D_OC_top$legend[5] <- "5 PARKINSON STUDY..."
#D_OC_top$legend[4] <- ""
#D_OC_top$legend[5] <- ""
#D_OC_top$legend[6] <- ""
#D_OC_top$legend[7] <- ""
...
D_OC_top$OC_rank <- seq(1,Top_n,1)

#Plot names of authors with Top_n offspring counts
P_OC_names <- ggplot(D_OC_top)
P_OC_names  <- P_OC_names  + geom_text(aes(x=rep(0, times=Top_n), y=rev(OC_rank), label=legend), color="black", family="Arial", size=Names_size, parse=F, hjust=0, nudge_x=-15)
...
P_AG <- P_AG + scale_y_continuous(limits = c(0.5,max(D_AG$AG)+0.5), breaks = seq(1,20, by = Tick_AG))
if(show_plots){P_AG}
...
P_AG_hist <- P_AG_hist + scale_x_continuous(limits = c(0.5,max(D_AG$AG)+0.5), breaks = seq(1,20, by = Tick_AG))
...
File_P_fam1 <- paste(User_dir_save, sprintf("%s_P_fam1.tif", Topic),sep="")
File_P_fam2 <- paste(User_dir_save, sprintf("%s_P_fam2.tif", Topic),sep="")

#Plot with labels
P_fam_tree_ratio <- 0.5
P_fam_tree_height <- max(D_OC_top$AG)*150
...
vertex.size=V(Net_fam_top)$degree_out*3,
edge.color="gray50",
edge.arrow.mode=0,
edge.width=2.0,
...
vertex.label=NA,
vertex.frame.color=V(Net_fam_top)$color,
vertex.size=V(Net_fam_top)$degree_out*4,
edge.color="gray50",
edge.arrow.mode=0,
edge.width=2.5,
layout=P_fam_layout_1,
...
D_CC_top_minus <- subset(D_CC_top, D_CC_top$Author_index <0)
D_CC_top_plus <- subset(D_CC_top, D_CC_top$Author_index >0)
...
#Generate plot
P_col <- ggplot()
P_col <- P_col + geom_curve(data = DT_col_net[N >= 1], aes(x = abs(L_AI)*-1, xend = abs(C_AI), y = L_PY_start, yend = C_PY_start, group = L_AI), color="grey80", size = 0.3, curvature = 0.4)

#Plot col top
P_col <- P_col + geom_curve(data = DT_col_net_top_out, aes(x = abs(L_AI)*-1, xend = abs(C_AI), y = L_PY_start, yend = C_PY_start, group = L_AI, color = L_TC), size = 0.4, curvature=-0.3)
P_col <- P_col + geom_curve(data = DT_col_net_top_in, aes(x = abs(L_AI)*-1, xend = abs(C_AI), y = L_PY_start, yend = C_PY_start, group = C_AI, color = C_TC), size = 0.4, curvature=-0.3)
...
P_col <- P_col + geom_text_repel(data = D_CC_top, aes(x = abs(Author_index)*-1, y = PY_start, label = CC_rank, color = Author_color), nudge_x = Tick_AI*-1.5, nudge_y = -10, vjust=0, hjust=1, size = Label_size, inherit.aes=FALSE)
...
File_P_col_norm <- sprintf("%s_P_col_norm.tiff", Topic)

#Offspring connections with normalized AI
P_col_norm <- ggplot()
P_col_norm <- P_col_norm + geom_line(data = D_all, aes(x = Author_index_norm, y = PY_start), size=0.5, color = "black")
P_col_norm <- P_col_norm + geom_point(data = D_CC_bottom, aes(x = Author_index_norm, y = PY_start, size = degree_all), color = col_norm, fill = "gray80", shape = 21, stroke = 1, alpha=Alpha_all)
P_col_norm <- P_col_norm + geom_point(data = D_CC_top, aes(x = Author_index_norm, y = PY_start, size = degree_all), fill = col_norm, shape = 21, color="black", alpha=Alpha_all, stroke = 1)
P_col_norm <- P_col_norm + scale_size_continuous(range = c(min(D_CC_bottom$degree_all), max(D_CC_top$degree_all)), breaks = c(min(D_CC_bottom$degree_all), max(D_CC_top$degree_all)))
P_col_norm <- P_col_norm + geom_text_repel(data = D_CC_top_minus, aes(x = Author_index_norm, y = PY_start, label = CC_rank), color = "black", nudge_x = 0, nudge_y = -17, hjust = 0, vjust = 0, size = Label_size, inherit.aes=FALSE)
P_col_norm <- P_col_norm + geom_text_repel(data = D_CC_top_plus, aes(x = Author_index_norm, y = PY_start, label = CC_rank), color = "black", nudge_x = 0, nudge_y = -10, hjust = 0, vjust = 0.5, size = Label_size, inherit.aes=FALSE)
P_col_norm <- P_col_norm + scale_fill_identity()
P_col_norm <- P_col_norm + scale_colour_identity()
P_col_norm <- P_col_norm + theme_bw()
P_col_norm <- P_col_norm + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black", size = Line_width))
P_col_norm <- P_col_norm + theme(legend.text = element_text(size = Legend_text_size), legend.background = element_blank(), legend.position =  c(0.85, 0.3), legend.title = element_blank(), legend.key=element_blank())
P_col_norm <- P_col_norm + theme(axis.ticks.length = unit(Tick_length, "cm"), axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_line(color = "black", size = Line_width))
P_col_norm <- P_col_norm + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
P_col_norm <- P_col_norm + scale_x_continuous(limits = c(-1, 1), breaks = seq(-1,1, by=0.2))
P_col_norm <- P_col_norm + scale_y_continuous(limits = c(PY_min,PY_max), breaks = seq(1700, 2040, by = Tick_years))
if(show_plots){P_col_norm}
if(save_plots){ggsave(File_P_col_norm, path = User_dir_save, height = Plot_height_PY_norm, width = Plot_width_AI_norm, units = "mm", dpi=300)}

File_P_CC_names <- sprintf("%s_P_CC_names.tiff", Topic)

#D_CC_top$legend[8] <- "8 EURO-MOTOR..."
...
if(show_plots){P_CC}
if(save_plots){ggsave(File_P_CC, path = User_dir_save, height = Plot_div_height*1.25, width = Plot_width_AI, units = "mm", dpi=300)}
...
if(save_plots){ggsave(File_P_CC_histo, path = User_dir_save, height = Plot_div_height*1.25, width = Plot_histo_width, units = "mm", dpi=300)}
...
P_PC_col <- P_PC_col + theme(legend.text = element_text(size = Legend_text_size), legend.position = Legend_position_1, legend.background = element_blank(), legend.title = element_blank())
...
if(save_plots){ggsave(File_P_PC_col, path = User_dir_save, height = Plot_div_height*1.25, width = Plot_width_AI, units = "mm", dpi=300)}
...
if(save_plots){ggsave(File_P_PC_col_histo, path = User_dir_save, height = Plot_div_height*1.25, width = Plot_histo_width, units = "mm", dpi=300)}
...
File_P_col_net <- paste(User_dir_save, sprintf("%s_P_col_net.tif", Topic),sep="")
...
D_TTP_bottom <- D_TTP_sorted[-(1:Top_n),]

D_TTP_first <- subset(D_all, abs(D_all$Author_index)==1)
P_TTP_top_norm_min <- round(min(D_TTP_top$PCxOCxCC_rel_max),3)
P_TTP_top_norm_max <- round(max(D_TTP_top$PCxOCxCC_rel_max),2)
P_TTP_norm_min <- round(min(D_TTP$PCxOCxCC_rel_max),4)
P_TTP_norm_max <- round(max(D_TTP$PCxOCxCC_rel_max),4)
P_TTP_top_min <- min(D_TTP_top$TTP_log)
P_TTP_top_max <- max(D_TTP_top$TTP_log)


#Prepare iTTP data for visualization
D_iTTP <- D_all[which(D_all$iPCxOCxCC > 1),]
D_iTTP$iTTP_log <- log10(D_TTP$iPCxOCxCC)

D_iTTP_sorted <- D_iTTP[order(-D_iTTP$iPCxOCxCC),]
D_iTTP_sorted$iTTP_rank <- seq_along(D_iTTP_sorted$iPCxOCxCC)
D_iTTP_sorted$legend <- paste(D_iTTP_sorted$iTTP_rank,D_iTTP_sorted$Author_name,sep=" ")

D_iTTP_top <- D_iTTP_sorted[1:Top_n,]

D_iTTP_top_minus <- subset(D_iTTP_top, D_iTTP_top$Author_index <0)
D_iTTP_top_plus <- subset(D_iTTP_top, D_iTTP_top$Author_index >0)
D_iTTP_bottom <- D_iTTP_sorted[-(1:Top_n),]

P_iTTP_top_norm_min <- round(min(D_iTTP_top$iPCxOCxCC_rel_max),3)
P_iTTP_top_norm_max <- round(max(D_iTTP_top$iPCxOCxCC_rel_max),2)
P_iTTP_norm_min <- round(min(D_iTTP$iPCxOCxCC_rel_max),2)
P_iTTP_norm_max <- round(max(D_iTTP$iPCxOCxCC_rel_max),2)
...
P_TTproduct <- P_TTproduct + geom_point(data = D_TTP, aes(x = Author_index, y = PY_start, color = Author_color, size = PCxOCxCC_rel_max), shape = 21, fill = "grey80", alpha=Alpha_all, stroke=1)
...
P_TTproduct <- P_TTproduct + geom_text_repel(data = D_TTP_top_minus, aes(x = Author_index, y = PY_start, label = TTP_rank, color = Author_color), nudge_x = Tick_AI*-1.5, nudge_y = 0, hjust = 0, vjust = 0, size = Label_size, inherit.aes=FALSE)
...
P_TTproduct <- P_TTproduct + scale_size_continuous(range = c(P_TTP_norm_min*30, P_TTP_norm_max*30), breaks = c(P_TTP_top_norm_min,P_TTP_top_norm_max))
...
P_TTproduct <- P_TTproduct + theme(legend.text = element_text(size = Legend_text_size), legend.position = c(0.8,0.3), legend.title = element_blank(), legend.background = element_blank(), legend.key=element_blank())
...
File_P_iTTP_names <- sprintf("%s_P_iTTP_names.tiff", Topic)

#D_iTTP_top$legend[4] <- "4 FUKUOKA KINKI..."

P_iTTP_names <- ggplot()
P_iTTP_names <- P_iTTP_names + geom_text(data=D_iTTP_top, aes(x=rep(0, times=Top_n), y=rev(iTTP_rank), label=legend), family="Arial", size=Names_size, parse=F, hjust=0, color = "black")
P_iTTP_names <- P_iTTP_names + theme_bw()
P_iTTP_names <- P_iTTP_names + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_blank())
P_iTTP_names <- P_iTTP_names + theme(legend.text = element_blank(), legend.title = element_blank(), axis.ticks = element_blank(), axis.title = element_blank(), axis.text = element_blank())
if(show_plots){P_iTTP_names}
if(save_plots){ggsave(File_P_iTTP_names, path = User_dir_save, height = Plot_names_height, width = Plot_names_width, units = "mm", dpi=300)}


File_P_TTP_norm <- sprintf("%s_P_TTP_norm.tiff", Topic)

#Offspring connections with normalized AI
P_TTP_norm <- ggplot()
P_TTP_norm <- P_TTP_norm + geom_line(data = D_all, aes(x = Author_index_norm, y = PY_start), size=0.5, color = "black")
P_TTP_norm <- P_TTP_norm + geom_point(data = D_TTP_bottom, aes(x = Author_index_norm, y = PY_start, size =  PCxOCxCC_rel_max), color = col_norm, fill = "gray80", shape = 21, stroke = 1, alpha=Alpha_all)
P_TTP_norm <- P_TTP_norm + geom_point(data = D_TTP_top, aes(x = Author_index_norm, y = PY_start, size =  PCxOCxCC_rel_max), fill = col_norm, shape = 21, color="black", alpha=Alpha_all, stroke = 1)
P_TTP_norm <- P_TTP_norm + scale_size_continuous(range = c(P_TTP_top_norm_min*30, P_TTP_norm_max*30), breaks = c(P_TTP_top_norm_min,P_TTP_top_norm_max))
P_TTP_norm <- P_TTP_norm + geom_text_repel(data = D_TTP_top_minus, aes(x = Author_index_norm, y = PY_start, label = TTP_rank), color = "black", nudge_x = -0.3, nudge_y = -15, hjust = 0, vjust = 0, size = Label_size, inherit.aes=FALSE)
P_TTP_norm <- P_TTP_norm + geom_text_repel(data = D_TTP_top_plus, aes(x = Author_index_norm, y = PY_start, label = TTP_rank), color = "black", nudge_x = 0.2, nudge_y = -5, hjust = 0, vjust = 0.5, size = Label_size, inherit.aes=FALSE)
P_TTP_norm <- P_TTP_norm + scale_fill_identity()
P_TTP_norm <- P_TTP_norm + scale_colour_identity()
P_TTP_norm <- P_TTP_norm + theme_bw()
P_TTP_norm <- P_TTP_norm + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black", size = Line_width))
#P_TTP_norm <- P_TTP_norm + theme(legend.text = element_text(size = Legend_text_size), legend.background = element_blank(), legend.position =  c(0.75, 0.4), legend.title = element_blank(), legend.key=element_blank())
P_TTP_norm <- P_TTP_norm + theme(legend.text = element_text(size = Legend_text_size), legend.background = element_blank(), legend.position =  c(0.2, 0.2), legend.title = element_blank(), legend.key=element_blank())
P_TTP_norm <- P_TTP_norm + theme(axis.ticks.length = unit(Tick_length, "cm"), axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_line(color = "black", size = Line_width))
P_TTP_norm <- P_TTP_norm + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
P_TTP_norm <- P_TTP_norm + scale_x_continuous(limits = c(-1, 1), breaks = seq(-1,1, by=0.2))
P_TTP_norm <- P_TTP_norm + scale_y_continuous(limits = c(PY_min,PY_max), breaks = seq(1700, 2040, by = Tick_years))
if(show_plots){P_TTP_norm}
if(save_plots){ggsave(File_P_TTP_norm, path = User_dir_save, height = Plot_height_PY_norm, width = Plot_width_AI_norm, units = "mm", dpi=300)}

File_P_iTTP_norm <- sprintf("%s_P_iTTP_norm.tiff", Topic)

#Offspring connections with normalized AI
P_iTTP_norm <- ggplot()
P_iTTP_norm <- P_iTTP_norm + geom_line(data = D_all, aes(x = Author_index_norm, y = PY_start), size=0.5, color = "black")
P_iTTP_norm <- P_iTTP_norm + geom_point(data = D_iTTP_bottom, aes(x = Author_index_norm, y = PY_start, size =  iPCxOCxCC_rel_max), color = col_norm, fill = "gray80", shape = 21, stroke = 1, alpha=Alpha_all)
P_iTTP_norm <- P_iTTP_norm + geom_point(data = D_iTTP_top, aes(x = Author_index_norm, y = PY_start, size =  iPCxOCxCC_rel_max), fill = col_norm, shape = 21, color="black", alpha=Alpha_all, stroke = 1)
#P_iTTP_norm <- P_iTTP_norm + scale_size_continuous(range = c(P_iTTP_norm_min*30, P_iTTP_norm_max*30), breaks = c(P_iTTP_norm_min,P_iTTP_top_norm_max))
P_iTTP_norm <- P_iTTP_norm + scale_size_continuous(range = c(0.3*20, 1*20), breaks = c(0.4,1.0))
P_iTTP_norm <- P_iTTP_norm + geom_text_repel(data = D_iTTP_top_minus, aes(x = Author_index_norm, y = PY_start, label = iTTP_rank), color = "black", nudge_x = -0.05, nudge_y = -17, hjust = 0, vjust = 0, size = Label_size, inherit.aes=FALSE)
P_iTTP_norm <- P_iTTP_norm + geom_text_repel(data = D_iTTP_top_plus, aes(x = Author_index_norm, y = PY_start, label = iTTP_rank), color = "black", nudge_x = 0.2, nudge_y = -8, hjust = 0, vjust = 0.5, size = Label_size, inherit.aes=FALSE)
P_iTTP_norm <- P_iTTP_norm + scale_fill_identity()
P_iTTP_norm <- P_iTTP_norm + scale_colour_identity()
P_iTTP_norm <- P_iTTP_norm + theme_bw()
P_iTTP_norm <- P_iTTP_norm + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black", size = Line_width))
P_iTTP_norm <- P_iTTP_norm + theme(legend.text = element_text(size = Legend_text_size), legend.background = element_blank(), legend.position =  c(0.8, 0.25), legend.title = element_blank(), legend.key=element_blank())
P_iTTP_norm <- P_iTTP_norm + theme(axis.ticks.length = unit(Tick_length, "cm"), axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_line(color = "black", size = Line_width))
P_iTTP_norm <- P_iTTP_norm + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
P_iTTP_norm <- P_iTTP_norm + scale_x_continuous(limits = c(-1, 1), breaks = seq(-1,1, by=0.2))
P_iTTP_norm <- P_iTTP_norm + scale_y_continuous(limits = c(PY_min,PY_max), breaks = seq(1700, 2040, by = Tick_years))
if(show_plots){P_iTTP_norm}
if(save_plots){ggsave(File_P_iTTP_norm, path = User_dir_save, height = Plot_height_PY_norm, width = Plot_width_AI_norm, units = "mm", dpi=300)}
...
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#Add-on script: analyses connections between topics of research
#See Pfrieger (2021) 
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#SETUP
...
D_all <- rbind(D_all_1,D_all_2,D_all_3,D_all_4,D_all_5)

Tot_number_authors <- nrow(D_all[!Duplicated(D_all$Author_name),])

Topic_number_authors <-aggregate(D_all$Author_name, by=list(Topic=D_all$Topic),FUN=length)


Topic_counts <- aggregate(D_all$Topic, by=list(Author_name=D_all$Author_name), FUN=length)
Topic_counts <- Topic_counts[order(-Topic_counts$x),]

Topic_counts_histo <- hist(Topic_counts$x, breaks=seq(1,6, by=1), plot=F, right=F)
D_hist_topic <- data.frame(x=Topic_counts_histo$mids-0.5, count=Topic_counts_histo$count, density=Topic_counts_histo$density)

File_P_topic_count <- sprintf("%s_P_topic_count.tiff", Topic)

#Plot histograms of PC values (last author: positive, first author: negative)
P_topic_counts <- ggplot()
P_topic_counts <- P_topic_counts + geom_col(data=D_hist_topic, aes(x, density), color="black", fill="black")
P_topic_counts <- P_topic_counts + theme_bw()
P_topic_counts <- P_topic_counts + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black", size = Line_width))
P_topic_counts <- P_topic_counts + theme(axis.ticks.length = unit(Tick_length, "cm"), axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_line(color = "black", size = Line_width))
P_topic_counts <- P_topic_counts + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
P_topic_counts <- P_topic_counts + scale_x_continuous(limits = c(0.5,5.5), breaks = seq(-500,500, by = 1))
P_topic_counts <- P_topic_counts + scale_y_continuous(limits = c(0,1), breaks = seq(0,1,by = 0.25))
if(show_plots){P_topic_counts}
if(save_plots){ggsave(File_P_topic_count, path = User_dir_save, height = Plot_workf_height, width = Plot_workf_height, units = "mm", dpi=300)}


#Determine intersection between Teams and collaborators per institution to establish links between institutions
#Last authors
Euler_1 <- D_all_1$Author_name
Euler_2 <- D_all_2$Author_name
Euler_3 <- D_all_3$Author_name
Euler_4 <- D_all_4$Author_name
Euler_5 <- D_all_5$Author_name

Euler_1vs2 <- unique(intersect(Euler_1, Euler_2))
D_1vs2 <- merge(D_all_1[which(D_all_1$Author_name %in% Euler_1vs2),c(1,14,49)],D_all_2[which(D_all_2$Author_name %in% Euler_1vs2),c(1,49)], by="Author_name")

Euler_1vs3 <- unique(intersect(Euler_1, Euler_3))
D_1vs3 <- merge(D_all_1[which(D_all_1$Author_name %in% Euler_1vs3),c(1,14,49)],D_all_3[which(D_all_3$Author_name %in% Euler_1vs3),c(1,49)], by="Author_name")

Euler_1vs4 <- unique(intersect(Euler_1, Euler_4))
D_1vs4 <- merge(D_all_1[which(D_all_1$Author_name %in% Euler_1vs4),c(1,14,49)],D_all_4[which(D_all_4$Author_name %in% Euler_1vs4),c(1,49)], by="Author_name")

Euler_1vs5 <- unique(intersect(Euler_1, Euler_5))
D_1vs5 <- merge(D_all_1[which(D_all_1$Author_name %in% Euler_1vs5),c(1,14,49)],D_all_5[which(D_all_5$Author_name %in% Euler_1vs5),c(1,49)], by="Author_name")

Euler_2vs3 <- unique(intersect(Euler_2, Euler_3))
D_2vs3 <- merge(D_all_2[which(D_all_2$Author_name %in% Euler_2vs3),c(1,14,49)],D_all_3[which(D_all_3$Author_name %in% Euler_2vs3),c(1,49)], by="Author_name")

Euler_2vs4 <- unique(intersect(Euler_2, Euler_4))
D_2vs4 <- merge(D_all_2[which(D_all_2$Author_name %in% Euler_2vs4),c(1,14,49)],D_all_4[which(D_all_4$Author_name %in% Euler_2vs4),c(1,49)], by="Author_name")

Euler_2vs5 <- unique(intersect(Euler_2, Euler_5))
D_2vs5 <- merge(D_all_2[which(D_all_2$Author_name %in% Euler_2vs5),c(1,14,49)],D_all_5[which(D_all_5$Author_name %in% Euler_2vs5),c(1,49)], by="Author_name")

Euler_3vs4 <- unique(intersect(Euler_3, Euler_4))
D_3vs4 <- merge(D_all_3[which(D_all_3$Author_name %in% Euler_3vs4),c(1,14,49)],D_all_4[which(D_all_4$Author_name %in% Euler_3vs4),c(1,49)], by="Author_name")

Euler_3vs5 <- unique(intersect(Euler_3, Euler_5))
D_3vs5 <- merge(D_all_3[which(D_all_3$Author_name %in% Euler_3vs5),c(1,14,49)],D_all_5[which(D_all_5$Author_name %in% Euler_3vs5),c(1,49)], by="Author_name")

Euler_4vs5 <- unique(intersect(Euler_4, Euler_5))
D_4vs5 <- merge(D_all_4[which(D_all_4$Author_name %in% Euler_4vs5),c(1,14,49)],D_all_5[which(D_all_5$Author_name %in% Euler_4vs5),c(1,49)], by="Author_name")


P_conn_width=280
P_conn_height=280

File_P_conn_net <- paste(User_dir_save, sprintf("%s_P_col_net.tif", Topic),sep="")

D_connect <- rbind(D_1vs2,D_1vs3,D_1vs4,D_1vs5,D_2vs3,D_2vs4,D_2vs5,D_3vs4,D_3vs5,D_4vs5)

D_connect_topic <- aggregate(D_connect$Author_name, by=list(Topic=D_connect$Topic.x), FUN=length)

D_connect_author <- aggregate(D_connect$Topic.x, by=list(Author_name=D_connect$Author_name), FUN=length)

D_connect_author <- D_connect_author[order(-D_connect_author$x),]
D_connect_author$Author_rank <- seq_along(D_connect_author$Author_name)

Top_n <- 5

D_connect_author_top <- D_connect_author[1:Top_n,]

D_connect_author_top$Author_color <- c("#411d00","#009916","#ff5386","#004784","#ff6d38")
D_connect_author_top$Author_color <- c("cyan3","chartreuse2","lightgoldenrod4","red3","purple3")

Conn_net <- graph.data.frame(d=D_connect[,c(3,4,1,2)], directed=F)
V(Conn_net)$degree_all <- degree(Conn_net, mode = "all", loops = FALSE, normalized = FALSE)
V(Conn_net)$color <- Topic_col[2:6]
V(Conn_net)$conn_fract <- V(Conn_net)$degree_all/Topic_number_authors$x
E(Conn_net)$Author_color <- ifelse(E(Conn_net)$Author_name %in% D_connect_author_top$Author_name, D_connect_author_top$Author_color, "grey")

if(save_plots){tiff(File_P_conn_net, width=P_conn_width, height=P_conn_height, units="mm", res=300, bg="transparent")}
plot.igraph(Conn_net,
vertex.frame.color=NA,
vertex.label=NA,
vertex.size=V(Conn_net)$degree_all/max(V(Conn_net)$degree_all)*50,
layout=layout_in_circle,
edge.color=E(Conn_net)$Author_color,
edge.width=ifelse(E(Conn_net)$Author_name %in% D_connect_author_top$Author_name,4,1)
)
if(save_plots){dev.off()}

File_P_names <- paste(User_dir_save, sprintf("%s_P_names.tiff", Topic),sep="")

File_P_names <- sprintf("%s_P_names.tiff", Topic)

#Plot names of authors connecting maximum number of topics
P_names <- ggplot(D_connect_author_top)
P_names  <- P_names  + geom_text(aes(x=rep(0, times=Top_n), y=Author_rank, label=Author_name, color=Author_color), family="Arial", size=Names_size, parse=F, hjust=0, nudge_x=-15)
P_names  <- P_names  + scale_color_identity()
P_names  <- P_names  + theme_bw()
P_names  <- P_names  + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_blank(), panel.background = element_blank(), plot.background = element_blank())
P_names  <- P_names  + theme(legend.text = element_blank(), legend.title = element_blank(), axis.ticks = element_blank(), axis.title = element_blank(), axis.text = element_blank())
if(show_plots){P_names}
if(save_plots){ggsave(File_P_names, path = User_dir_save, height = Plot_names_height/2, width = Plot_names_width, units = "mm", dpi=300)}
